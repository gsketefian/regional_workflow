...
#-----------------------------------------------------------------------
#
# Make sure that SUB_HOURLY_POST and, if applicable, DT_SUBHOURLY_POST_MNTS 
...
# greater than or equal to the forecast model's main time step DT_ATMOS.
#
#  dt_subhourly_post_secs=$(( DT_SUBHOURLY_POST_MNTS*60 ))
#echo "BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB"
#echo "dt_subhourly_post_secs = ${dt_subhourly_post_secs}"
#  if [ "${dt_subhourly_post_secs}" -lt "${DT_ATMOS}" ]; then
#    print_err_msg_exit "\
#When performing sub-hourly post (i.e. SUB_HOURLY_POST set to \"TRUE\"), 
#the time interval specified by DT_SUBHOURLY_POST_MNTS (after converting 
#to seconds) must be greater than or equal the time step DT_ATMOS used in
#the forecast model.  In this case, it is not:
#  SUB_HOURLY_POST = \"${SUB_HOURLY_POST}\"
#  DT_SUBHOURLY_POST_MNTS = \"${DT_SUBHOURLY_POST_MNTS}\" (in minutes)
#  dt_subhourly_post_secs = \"${dt_subhourly_post_secs}\" (above value in seconds)
#  DT_ATMOS = \"${DT_ATMOS}\" (in seconds)"
#
# Check that DT_SUBHOURLY_POST_MNTS (after converting to seconds) is 
# evenly divisible by the forecast model's main time step DT_ATMOS.
#
  rem=$(( DT_SUBHOURLY_POST_MNTS % DT_ATMOS ))
#  rem=$(( dt_subhourly_post_secs % DT_ATMOS ))
#  rem=$(( dt_subhourly_post_secs % 10#${DT_ATMOS} ))
  if [ ${rem} -ne 0 ]; then
    print_err_msg_exit "\
When performing sub-hourly post (i.e. SUB_HOURLY_POST set to \"TRUE\"), 
the time interval specified by DT_SUBHOURLY_POST_MNTS (after converting 
